...
function Todaybook() {
  ...
  useEffect(() => {
    const todaybookAPI = async () => {
      try {
        const response = await axios.get('http://study.aiclub.kr:8007/today/book');
  ...
  return (
    <div className="todaybook-wrap">
      <div className='todaybook-title'>오늘의 책</div>
      {item.length > 0 ? (
          ...
          {item.map((book, index) => (
            <div key={index} className='book-location'>
                ...
                <div className='book-information'>
                  ...
                  <div className='book-desd'>{truncateText(book.book_desd, 100)}</div>
                </div>
